...
@Component({
    ...
    <div *ngIf="product" class="description-modal-overlay" (click)="closed.emit()">
      <div class="description-modal" (click)="$event.stopPropagation()">
        ...
        <div class="modal-body">
          <div class="lotes-header" *ngIf="!product.parentId"><button class="btn-create-lote" type="button" (click)="showLotes = true">Gestionar Lotes</button></div>
  ...
  styles: [`
    ...
    .modal-body { max-height: 70vh; overflow-y: auto; padding: 1rem 1rem 1.5rem; color: #0f172a; }.lotes-header { display: flex; justify-content: flex-end; margin-bottom: .75rem; }.btn-create-lote { padding: .5rem .75rem; border: 1px solid #c7d2fe; border-radius: 7px; background: #eef2ff; color: #4338ca; font-size: .8rem; font-weight: 700; cursor: pointer; }
    ...
    @media (max-width: 480px) { .modal-header, .modal-body { padding: 1rem; }.modal-info-grid { grid-template-columns: 1fr; }.modal-footer { padding: .85rem 1rem 1rem; } }
  `]
})
