...
export class AuthService {
  ...
  login(username: string, password: string): Observable<boolean> {
    const apiUrl = this.apiConfig.getApiUrl('/api/auth/login');
    console.log('Enviando login a:', apiUrl, 'con usuario:', username);
  ...
  register(username: string, password: string): Observable<boolean> {
    const apiUrl = this.apiConfig.getApiUrl('/api/auth/register');
    console.log('Enviando registro a:', apiUrl, 'con usuario:', username);
    return this.http.post(apiUrl, { username, password }, { responseType: 'text' }).pipe(
      tap(response => {
